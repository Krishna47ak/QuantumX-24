  ...
  return (
    <div>
      ...
      >
        ...
        <Image
          className="rounded-xl w-full object-fill shadow-md shadow-indigo-600"
          src={data?.img}
          width={250}
          height={250}
          alt="logo"
      ...
      {onCard && from != "home" && (
        <div className="fixed flex justify-center items-center py-32 z-20 bg-[#0000004d] top-0 bottom-0 left-0 right-0">
          <div
            ...
            className="relative flex min-[1293px]:space-x-5 bg-[#0f172ad9] backdrop-blur-md w-[85vw] h-[30rem] md:h-[35rem] mt-16 rounded-3xl md:px-10 py-7 pt-10 md:py-12 lg:py-16"
          >
            <Image
              onClick={() => setOnCard(false)}
              className="absolute right-5 md:right-9 top-2 md:top-5 cursor-pointer"
              src="/close.svg"
              width={24}
              height={24}
              alt="close"
            ...
            <div className="flex flex-col justify-between py-1 overflow-x-auto px-7 md:px-0">
              <div>
                <div className="text-white sm:text-lg text-justify">
                  {data?.desc}
                </div>
                <div className="bg-[#fafafa33] min-[853px]:flex justify-between items-end text-white p-5 mt-5 md:mt-10 rounded-xl">
                  <div>
                    <p className="sm:text-lg font-medium text-slate-400 font-mono">
                      ...
                      data?.id != "QX_EV_07" && (
                        <>
                          {(data?.fixedSize || data?.minSize) && (
                            <div className="sm:text-lg font-medium text-slate-400 mt-2 font-mono">
                              Team size :{" "}
                              {data?.fixedSize
                                ? data?.fixedSize === 1
                                  ? "Individual"
                          ...
                          <p className="sm:text-xl font-medium text-[#fff523] mt-2 font-mono">
                    ...
                    {data?.contact && (
                      <div className="flex items-start sm:items-center mt-2">
                        <Image
                          ...
                          alt="location"
                        />
                        <p className="ml-2 text-lg sm:text-xl font-medium text-slate-400 font-mono">
                      ...
                      data?.id != "QX_EV_07" && (
                        <div className="flex items-center mt-2">
                          ...
                          <a
                            href={data?.ruleBook}
                            target="_blank"
                            className="ml-2 text-blue-600 text-lg sm:text-xl font-medium font-mono underline cursor-pointer select-none"
                          >
                            {from != "workshop" ? "Rule book" : "Details"}
                          </a>
                        </div>
                      )}
                  </div>
